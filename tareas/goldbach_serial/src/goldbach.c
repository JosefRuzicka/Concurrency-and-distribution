...
int64_t* sieveOfEratosthenes(int64_t largestInputNumber) {
  ...
  bool isPrime = true;
	...
	for (primeNumbersIndex = 2; primeNumbersIndex <= largestInputNumber;
		...
		// Adapted from https://technotip.com/7469/c-program-to-find-prime-numbers-from-1-to-300-using-for-loop/ 
		for (int dividend = 2; dividend <= sqrtl(primeNumbers[primeNumbersIndex]); dividend++){
			if (((llabs(primeNumbers[primeNumbersIndex]) %dividend == 0) && (llabs(primeNumbers[primeNumbersIndex]) != dividend))){
				isPrime = false;
				goto nextIteration;
			}
		}
